...
const { DISCORD_TOKEN, DISCORD_CLIENT_ID, DISCORD_GUILD_ID, ADMIN_ROLE_ID, MOD_ROLE_ID } =
  process.env;
...
const command = new SlashCommandBuilder()
  ...
  .addSubcommand((sub) =>
    sub
      ...
      .addStringOption((option) =>
        option.setName("word").setDescription("Word to add").setRequired(true)
      )
  )
  .addSubcommand((sub) =>
    sub
  ...
  .addSubcommand((sub) => sub.setName("listblacklist").setDescription("List blacklist words"))
  .addSubcommand((sub) =>
    sub
      .setName("removeblacklist")
      .setDescription("Remove a word from the blacklist")
      .addStringOption((option) =>
        option.setName("word").setDescription("Word to remove").setRequired(true)
      )
  )
  .addSubcommand((sub) => sub.setName("restart").setDescription("Restart the bot (admin only)"))
  .addSubcommand((sub) => sub.setName("stop").setDescription("Stop the bot (admin only)"))
  .addSubcommand((sub) =>
    sub
      .setName("warn")
      .setDescription("Warn a Twitch user")
      .addStringOption((option) =>
        option.setName("username").setDescription("Twitch username to warn").setRequired(true)
      )
      .addStringOption((option) =>
